...
| Japan Public Procurement Information Portal (KKJ) search API | Implementation ready; production disabled | Official XML search API | Terms permit external API use with source notice and portal link; paid redistribution needs written confirmation | On-demand only, safe metadata only, no bulk harvesting, preserve source URL and attribution, keep feature flag off until confirmation |
